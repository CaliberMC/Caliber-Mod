...
public class WoodcutterMenu extends AbstractContainerMenu {
   ...
   public WoodcutterMenu(int pContainerId, Inventory pPlayerInventory, final ContainerLevelAccess pAccess) {
      super(CaliberMenuTypes.WOODCUTTER.get(), pContainerId);
      this.access = pAccess;
   ...
   public boolean stillValid(Player pPlayer) {
      return stillValid(this.access, pPlayer, CaliberBlocks.WOODCUTTER.get());
   }
   ...
   public void slotsChanged(Container pInventory) {
      ...
      if (!itemstack.is(this.input.getItem())) {
         ...
         if (!itemstack.isEmpty()) {
            this.recipes = this.level.getRecipeManager().getRecipesFor(CaliberRecipeSerializers.WOODCUTTING_TYPE.get(), pInventory, this.level);
         }
      }
   ...
   public ItemStack quickMoveStack(Player pPlayer, int pIndex) {
      ...
      if (slot.hasItem()) {
         ...
         } else if (this.level.getRecipeManager().getRecipeFor(CaliberRecipeSerializers.WOODCUTTING_TYPE.get(), new SimpleContainer(itemstack1), this.level).isPresent()) {
