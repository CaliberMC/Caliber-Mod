...
public class CaliberBlocks {
    ...
    public static final List<ModBlockFamily.Variant> COMPAT_PLANK_VARIANTS = createCompatPlankVariants();
    
    private static List<ModBlockFamily.Variant> createCompatPlankVariants() {
        List<ModBlockFamily.Variant> COMPAT_PLANK_VARIANTS = new ArrayList<>(PLANK_VARIANTS);
        COMPAT_PLANK_VARIANTS.remove(ModBlockFamily.Variant.TALL_DOOR);
        return List.copyOf(COMPAT_PLANK_VARIANTS);
    }

    public static final RegSupplier<Block> WOODCUTTER = registerBlock("woodcutter",
    ...
    private static <T extends Block> RegSupplier<T> registerBlockWithoutBlockItem(String name, Supplier<T> block) {
        blockCount++;
        return RegHelper.registerBlock(new ResourceLocation(Caliber.MOD_ID, name), block);
    }

    public static <T extends Block> RegSupplier<T> registerBlock(String name, Supplier<T> block) {
        RegSupplier<T> toReturn = RegHelper.registerBlockWithItem(new ResourceLocation(Caliber.MOD_ID, name), block);
        blockCount++;
        return toReturn;
    }
